...
training a model on concatenated VGG and audio data
...
X_dataVGG = StringIO(file_io.read_file_to_string('gs://mediaeval_data_storage/DevSet01/VGG16_data01.npy'))
#X_data = StringIO(file_io.read_file_to_string('gs://mediaeval_data_storage/DevSet01/audio_data01.npy'))

X_inputVGG_long = np.load(X_dataVGG)

X_dataAudio = StringIO(file_io.read_file_to_string('gs://mediaeval_data_storage/DevSet01/audio_data01.npy'))

X_inputAudio_long = np.load(X_dataAudio)

X_input_long = np.concatenate((X_inputVGG_long, X_inputAudio_long), axis=2)
print(X_input_long.shape)
...
filename = 'modelVGG_Audio_Concat'
