...
# VGG data
X_data_VGG = StringIO(file_io.read_file_to_string('gs://mediaeval_data_storage/TestSet/VGG16_test.npy'))
...
X_predict_VGG = X_reshape_VGG[:, :timesteps, :]


# AUDIO data
#X_data = StringIO(file_io.read_file_to_string('gs://mediaeval_data_storage/DevSet02/VGG16_data02_2.npy'))
#X_data = StringIO(file_io.read_file_to_string('gs://mediaeval_data_storage/TestSet/audio_test.npy'))
...
out = run_model.predict([X_predict_VGG, X_predict_VGG, X_predict_VGG, X_predict_VGG])

# round the predictions
round_out = np.round(out)

# reshape the predictions
round_out = np.reshape(round_out, (X_input_long_VGG.shape[0], 6262))

# find the fear-inducing seconds with the np.nonzero command
#count = 0
#for mov in round_out:
    #print("mov {}:".format(count))
    #print(np.nonzero(mov))
    #count += 1
...
#for i in range(12):
    #print(stored_out[i])
#filename = 'output_predictions_run1'
#path = 'gs://mediaeval_data_storage'
#np.save(filename, stored_out)
#with file_io.FileIO(filename, mode='r') as inputFile:
        #with file_io.FileIO(path + '/' + filename, mode='w+') as outFile:
            #outFile.write(inputFile.read())
            
np.save(file_io.FileIO('gs://mediaeval_data_storage/prediction_run3', 'w'), stored_out)
